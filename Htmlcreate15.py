...
# This module provides a class-based interface (`Html_Create`) to create Leaflet-based HTML maps with support for interactive tracing and visualization of RIPE Atlas results.

### Key Functions

#- **`create_header_html()`**
#  - Initializes HTML structure, JavaScript variables, and base map.
#  - Links to Leaflet libraries and adds target metadata.

#- **`create_target()` and `create_probes()`**
#  - Plot target and source probes on the map with tooltips.

#- **`create_hop()`**
#  - Places markers for each hop, showing IP, ASN, and RTT.
#  - Distinguishes between regular and IXP-related hops.

#- **`create_lines_var()`**
#  - Draws colored lines between hops.
#  - Visualizes RTT and hop transition information.

#- **`create_greater()`**
#  - Draws a radius around a source probe based on final RTT.
#  - Used for RTT-based error estimation.

#- **`create_ixp()`**
#  - Visualizes entry and exit into Internet Exchange Points.
#  - Connects facilities with inferred path segments.

#- **`create_target_area()`**
#  - Draws the estimated area where the target could be located based on RTT delta from the IXP.

#- **`create_layer_checker()`**
#  - Adds layer toggles for filtering visualized probes by IXP group.
