...
def get_hop_location(prev_hop,hop,probe_id,source_coords): #rtt
    ...
    # crappy quick fix for when rtt is greater than final rtt
    if this_fraction >= 1:
        this_fraction = 1.0
        y1 = round(source_coords[0],6)-.0001
        x1 = round(source_coords[1],6)-.0001
        source_coords = [y1,x1]
    ...
    '''
    print(this_fraction,this_rtt, rtt, last_rtt, default_rtt, source_coords)
    
    '''
    y2 = round(target_coords[0],6)
    x2 = round(target_coords[1],6)
    
    target_coords = [y2,x2]
    '''
    print(this_fraction,this_rtt, rtt, last_rtt, default_rtt, source_coords, target_coords )
...
'100.124.', '100.125.', '100.126.', '100.127.']
    ...
    #with open("results/target_6695_jisc.json") as f:
        measurements =json.load(f)

    #report_file= "results/6087_report.json"
    #report_file= "results/6716_report.json"
    report_file= "results/6087_v15_report.json"
        ...
        for probe_id in measurements[measurement_id]['results']:
            ...
            if probe_id != this_target:
                ...
                for hop in measurements[measurement_id]['results'][probe_id]['hops']:
                    ...
                    # rtt = measurements[measurement_id]['results'][probe_id]['hops'][hop]['rtt']
                    this_ip, this_hop, this_rtt, lat1, lon1, lat2, lon2, hop_distance = get_hop_location(prev_hop,hop,probe_id,source_coords) #rtt
                    ...
                    if this_ixp and not ixp_entered_flag:
                        ...
                        if this_network['netfac_set']:
                            ...
                            for netfac in this_network['netfac_set']:
                                ...
                                print('and its NETFAC INFO is', netfac_info[0])
                        ...
                        if len(possible_entry_facility) > 1:
                            ...
                            # if list of possible entry facilities are all equinix in slough then just chose the initial one   
                                ...
                                results[probe_id]['status_reason'] = 'initial Facility used as no valid rule for the Facility entry list ' + str(possible_entry_facility)
                            ...
                            results[probe_id]['status'] = False
                            results[probe_id]['status_reason'] = probe_id+' No entry facility exists set it to 832 temporarily)'
                            results[probe_id]['status_code'] = 6
                            ixp_entry_point = 832
                        ...
                        if ipaddress.ip_address(this_hop['from']) not in ipaddress.ip_network(ixps_uk[ixp_entered_id]['ipv4_prefix']):
                            ...
                            if this_network['netfac_set']:
                                ...
                                # add JANET additional manually found facilities TODO why arnt these found ?
                                    ...
                                    #print('THIS IS IT * The ASN succeding the IXP is', hop_asn, ' and its facilites are', exit_netfac_ids)
                                    #input('wait')
                            ...
                            if len(possible_exit_facility) > 1:
                                ...
                                # if list of possible exit facilities are all equinix in slough then just chose the initial one   
                                    ...
                                    results[probe_id]['status'] = False
                                    results[probe_id]['status_reason'] = 'initial Facility used as no valid rule for the Facility EXIT list ' + possible_exit_facility
                                ...
                                results[probe_id]['status'] = False
                                results[probe_id]['status_reason'] = probe_id+' does not have an EXIT POINT'
                                results[probe_id]['status_code'] = 4
                ...
                for hop in ixp_pre_hops:
                    ...
                    if facilities_used[probe_id]:
                        ...
                        results[probe_id]['status'] = False
                        results[probe_id]['status_reason'] = 'This probe doesnt use an IXP '+ probe_id
                        results[probe_id]['status_code'] = 1
                ...
                for hop in ixp_in_hops:
                    ...
                    results[probe_id]['exit_fac'] = facilities_used[probe_id][2]
                    results[probe_id]['exit_rtt'] = this_rtt
                    results[probe_id]['time_across_ixp'] = results[probe_id]['exit_rtt'] - results[probe_id]['entry_rtt']
                ...
                for hop in ixp_post_hops:
                    ...
                    if error_radius <= 0:
                        results[probe_id]['status'] = False
                        results[probe_id]['status_reason'] = 'The rtt difference at the target is negative '+ probe_id
                        results[probe_id]['status_code'] = 3
