...
# Uses the results/targetsfull.json file created by read_measurments.py, can use the entire file but to limit
...
def get_hop_location(ixp_pre_hops,ixp_in_hops,ixp_post_hops,facilities_used,this_target,probe_id,prev_hop,hop,hop_details,ixp_entered_flag, ix_hop,max_hop):
        ...
        if ipaddress.ip_address(this_hop['ip_from']) in ipaddress.ip_network('195.66.236.0/22'):
            ...
            try:
                ...
                try:
                    ...
                    this_hop['asn'] = 0
                    input('this ip is an ix prefix but couldnt get an asn',ixp_ip)
                    input('wait 579')
                ...
                try:
                    ...
                    try:
                        ...
                        if len(this_facility) == 0:
                            ...
                            input('trying a reverse traceroute where facilities = 0 line 858')
    ...
    if ix_prefix_flag:
        ...
        #print('FAC IS',this_fac)
            ...
            input('couldnt find a this_fac excepted',this_hop['ip_from'])
            results['rule5.fac_to_ip_table']+=1
            ...
            input('wait on exit facilities 1442')        
...
def get_facilitys(rules,rname,this_hop):
    ...
    if len(f) > 1:
        ...
        if all(x in large_london_ger_list for x in f):
            ...
            input('multiple facilities found')
    elif len(f) == 0:
...
def main(argv):
    ...
    # never mind its USELESS DATA, only top level domains
    '''
    valid_domains = {}
    ...
    '''
    caida_ips = set()
    my_caida = open('data/CAIDA/caida', 'w')
    ...
    with open('data/CAIDA/midar-iff.nodes', mode = 'r') as csv_file:
        ...
        for row in csvreader:
            ...
            row_count +=1
    my_caida.close()
    print('ips are',ip_count)
    print(caida_ips)
    print(len(caida_ips))        
    #caida_json = jsonpickle.encode(caida_ips)
    #with open('data/CAIDA/caida.json', 'w') as f:
        #json.dump(caida_json, f)                   
    print('Caida Ips written to data/CAIDA/caida.json',)
    input('wait')
    '''
    ...
    def insintotablenode(node,ip):
        try:
            ...
            if sqliteConnection:
                ...
                #print("The SQLite connection is closed")
    '''
    #cursor.execute("""CREATE TABLE ips (id INTEGER PRIMARY KEY AUTOINCREMENT, ip varchar(15), node varchar(15))""")
    #create CAIDA table
    ...
    con.commit()
    cursor.execute("""CREATE TABLE ips (node varchar(15) PRIMARY KEY , ips varchar(255))""")
    con.commit()
    ...
    with open('data/CAIDA/gb_nodes.json', 'w') as f:
    ...
    with open('data/CAIDA/midar-iff.nodes') as f:
