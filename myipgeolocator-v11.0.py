...
def reverse_traceroute(source,target,measurement):
    print ('Source',source)
    print('target',target)
...
def get_hop_location(ixp_pre_hops,ixp_in_hops,ixp_post_hops,facilities_used,this_target,probe_id,prev_hop,hop,hop_details,ixp_entered_flag):
    ...
    if this_hop['id'] == '1': 
        ...
        try:
            ...
            #input('wait')
            this_facility, coords = get_coords(this_hop,rname,probe_id,this_target)
        ...
        if results[probe_id]['status'] != True:
            ...
            if len(possible_exit_facility) > 1:
                ...
                # if list of possible exit facilities are all equinix in slough then just chose the initial one   
                    ...
                    this_hop['hop_longitude'] = facilitys_uk[str(ixp_exit_point[0])]['longitude']
                    results[probe_id]['status'] = True 
                    results[probe_id]['status_reason'].append(probe_id+' All Equinix so chose first one'+ixp_exit_point[0])
                    results[probe_id]['status_code'].append(0)
                    append_vptable_dict(3, this_hop['ip_from'],this_hop['hop_latitude'], this_hop['hop_longitude'])
                    stats['3'] += 1
                    # if there are multiple shared facilities for the AS and the IX
...
def get_coords(this_hop,rname,probe_id,this_target):
    ...
    if not already_have_possible_facility:
        ...
        if len(f) > 1:
            ...
            if f[0] == '896' and f[1] == '2384':
                ...
                this_measurement= str(my_measurements[probe_id]['measurement'])
                print('The measurement is', this_target,probe_id,this_measurement)
                reverse_measurment = my_targets[this_measurement]['results'][this_target]
                print('The source_emasurment is ', reverse_measurment)
                
                this_fac = reverse_traceroute(this_target,probe_id,reverse_measurment)
                print(this_fac)
                if this_fac:
...
def main(argv):
    ...
    global facilitys_uk
    # Read in the UK Facilities records
    # TODO NOTE THERE IS A NEW LIST (as of 16/2/2023) OF UK facilities at peeringdb_test_results/uk_facilities_new.json that really needs
    # to be integrated into this list
