...
def get_hop_location(ixp_pre_hops,ixp_in_hops,ixp_post_hops,facilities_used,this_target,probe_id,prev_hop,hop,hop_details,ixp_entered_flag, ix_hop,max_hop):
        ...
        if this_hop['rtt'] < 1:
            ...
            if not this_hop['ip_from'] == measurement[this_target] ["probe_ip"]: #make sure this isnt the target ip address
                ...
                try:
                    ...
                    # input('wait')
                    try:
                        this_facility, coords, rules = get_coords(rules,this_hop,rname,probe_id,this_target,rule,1,max_hop)
                        print('kkk',rules, this_facility,coords)
                        # input('wait')
                        # TODO ##### DONE #### might be worth carrying out a RTT check less than .5 to ensure gateway is local
                        if this_facility:
                            lon = coords[0]
                            lat = coords[1]
            ...
            try:
                ...
                try:
                    this_facility, coords, rules = get_coords(rules,this_hop,rname,probe_id,this_target,rule,1,max_hop)
                    print('got facility and coords', this_facility,coords)
                    #input('wait')
                    # TODO ##### DONE #### might be worth carrying out a RTT check less than .5 to ensure gateway is local
                    if this_facility:
                        lon = coords[0]
    ...
    if prev_hop['use_next_hop_loc'] == True and not ix_prefix_flag: # make sure this hop isnt the IX hop as that is at the far end of the link
        ...
        try:
            ...
            this_facility,coords,rules = get_coords(rules,this_hop,rname,probe_id,this_target,rule,ix_hop,max_hop)
            this_hop['rdns'] = rname
            ...
            if this_facility:
                ...
                this_hop['hop_latitude'] = lat
                append_vptable_dict(2, this_hop['ip_from'],this_hop['hop_latitude'], this_hop['hop_longitude'],rname,this_facility,port,speed)
                if rname.casefold() in results['successes']['rule3']:
                    results['successes']['rule3'][rname.casefold()] +=1 
                    results['successes']['rule3']['total'] +=1 
                else:
                    results['successes']['rule3'][rname.casefold()] = 1
                    results['successes']['rule3']['total'] +=1 
