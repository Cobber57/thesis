...
# requries my Htmlcreate.py 
if __name__ == "__main__":
    
    from Htmlcreate2 import Html_Create # my htmlcreate module
        ...
        for probe_id in measurements[measurement_id]['results']:
            ...
            if probe_id != this_target:
                ...
                for hop in measurements[measurement_id]['results'][probe_id]['hops']:
                    ...
                    if r.status_code == 200:
                        ...
                        for attrib in attributes:
                            if attrib['name'] == 'origin':
                                ...
                                print(ripedb_asn, response.asns,asns )
                    ...
                    # If the hop doesnt have an asn because it is a local subnet then set it to the last previous ASN plus other random checks
                    if not this_hop['asn']:
                        ...
                        # TODO: I bet these can be removed now since moving to the RIPE database instead of RIPE stat
                        
                        print(this_hop['from'],this_hop['asn'] )
                        if ipaddress.ip_address(this_hop['from']) in ipaddress.ip_network('37.143.136.0/24'):
                            this_hop['asn'] = '196745'
                        elif ipaddress.ip_address(this_hop['from']) in ipaddress.ip_network('138.197.249.0/24'):
                            this_hop['asn'] = '14061'
