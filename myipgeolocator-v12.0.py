...
def reverse_traceroute(source,target,measurement,rdns,rule,ix_hop,max_hop):
    print ('Source',source)
    print('target',target)
    print('measurement',measurement)
    facility = None
    reverse_hop = int(max_hop)-int(ix_hop)
    for hop in measurement['hops']:
        if int(hop) == reverse_hop: # only check reverse traceroute if this hop is tthe correct hop
            try:
                this_ip = measurement['hops'][hop]['ip_from']
                ixinfo = ix_detail_dict[this_ip]
                print('ixinfo is' , this_ip, ixinfo)
                facility = ixinfo['facility_number']
                rdns = 'LINX '+ ixinfo['Peering LAN']
                rule.append('reverse_tr')
                break
                # raise UnboundLocalError('ixinfo was found')
            except:
                pass 
            '''  
            finally:
...
def get_hop_location(ixp_pre_hops,ixp_in_hops,ixp_post_hops,facilities_used,this_target,probe_id,prev_hop,hop,hop_details,ixp_entered_flag, ix_hop,max_hop):
    ...
    # individual ip addresess lookups  
    print(this_hop['ip_from'])
    ...
    if this_hop['id'] == '1': 
        if not this_hop['ip_from'] == measurement[this_target] ["probe_ip"]: #make sure this isnt the target ip address
            ...
            try:
                ...
                #input('wait')
                this_facility, coords, rule = get_coords(this_hop,rname,probe_id,this_target,rule,ix_hop,max_hop)
                ...
                if this_facility:
                    ...
                    # TODO: #THIS IS NOW DONE.# We actually need to get the prefix downloaded from the probe so we can test against their correct subnets
                    ...
                    # recreate the uk-measurements file with the prefix field added. DONE 
                    ...
                    '''
                    if ipaddress.ip_address(this_hop['ip_from']) in ipaddress.ip_network(uk_probes[int(probe_id)]["prefix"]):
                        ...
                        # TODO there seems to be 2 probes with the wrong gateway address 6471 and 6501, need to look at these.
                        print ('hmm hop is 1 but subnets dont match (2)- see the find location logic rules 1 - may need a new rule')
                        print (' this simply may be that the router is not returning the incoming ip adddress but a default ip address')
                ...
                print('RULE1 and excepted')
            finally:
                rule.append(1)  
        else:
            print('was target address')
        input('wait')
                ...
                for h in range(int(hop)-1,0,-1):
                    ...
                    if results[probe_id][str(h)]['use_next_hop_loc'] == True:
                        ...
                        if not results[probe_id][str(h)]['local_subnet_flag']:
                            ...
                            append_vptable_dict(4, results[probe_id][str(h)]['ip_from'],this_hop['hop_latitude'], this_hop['hop_longitude'],rname,this_hop['facility'], "","")
    ...
    if ix_prefix_flag:
        ...
        try:
            ...
            results[probe_id]['status'] = True 
            results[probe_id]['status_reason'].append(probe_id+' Exit Facility is '+ixp_exit_point[0])
            results[probe_id]['status_code'] = 0
        ...
        # below are examples of how to place the hops on the map - not implemented
    ...
    if not rule_flag:
        ...
        try:
            ...
            if this_facility:
                ...
                lat = coords[1]
                this_hop['facility'].append(this_facility)
                print('RULE 6 ',this_hop['facility'],this_facility)
                this_hop['network'] = 0
                this_hop['hop_longitude'] = lon
                this_hop['hop_latitude'] = lat
                append_vptable_dict(2, this_hop['ip_from'],this_hop['hop_latitude'], this_hop['hop_longitude'],rname,this_hop['facility'],"","")
            ...
            print(' RULE 6 couldnt finda  rdns name ',rname,this_hop)
            append_vptable_dict(5, this_hop['ip_from'],this_hop['hop_latitude'], this_hop['hop_longitude'],rname,this_hop['facility'],"","")
    ...
    return this_hop, facilities_used, ixp_pre_hops,ixp_in_hops,ixp_post_hops,rule,ix_hop

def get_coords(this_hop,rname,probe_id,this_target,rule,ix_hop,max_hop):
    ...
    if not skip_regex_check: 
        for this_rdns_partial_name in rdns_parts_list:
            ...
            for this_part in rdns_partial_list:
                ...
                red
                '''
                print('this part is', this_part, rdns_partial_list)
                ...
                if this_part == 'faraday':
                    ...
                    already_have_possible_facility = True
                    print(possible_facilitys)
                    rule.append('get_coords BT Faraday')
    ...
    if not already_have_possible_facility:
        ...
        if len(f) > 1:
            ...
            if f[0] == '896' and f[1] == '2384':
                ...
                print('The source_emasurment is ', reverse_measurment)
                #==================================================================================
                this_fac,this_hop['rdns'],rule = reverse_traceroute(this_target,probe_id,reverse_measurment,this_hop['rdns'],rule,ix_hop,max_hop)
                print(this_fac)
                #input('wait')
            ...
            for probe_id in measurements[measurement_id]['results']:
                ...
                if probe_id != this_target:
                    ...
                    for hop in measurements[measurement_id]['results'][probe_id]['hops']:
                        ...
                        print('HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH') 
                        this_hop, facilities_used, ixp_pre_hops,ixp_in_hops,ixp_post_hops,rule,ix_hop = get_hop_location(ixp_pre_hops,ixp_in_hops,ixp_post_hops,facilities_used,this_target,probe_id,prev_hop, hop, this_hop_results,ixp_entered_flag,ix_hop,max_hop)

                        print(this_hop)
                        
                        prev_hop = this_hop
                        results[probe_id][hop] = this_hop
                        results[probe_id][hop]['rule_use'] = rule
                        print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
                        #print('Vptable is ',vptable_dict)
                        print('Rules are',rule)
                        input('wait')
                    print(results[probe_id])
