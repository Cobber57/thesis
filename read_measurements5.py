# This version was written  to read in the measurments from the 650 probes used in the latest 76 measurements
# however it is currently crashing at various probes. not sure why. Too much data ???
...
if __name__ == "__main__":
    ...
    for target_probe in measurements:
        ...
        for traceroute in results:
            ...
            if str(source_probe) not in  measurements:
                not_in_set.add(source_probe)
                notin +=1
                #print('NOT IN', not_in_set,notin)
                #input('wait')
                continue
    ...
    outfile.close()
    with open("results/targetslatest_notin.json", 'w') as outfile:
        json.dump(not_in_set, outfile)
    outfile.close()
